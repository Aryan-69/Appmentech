...
5. **Solutions** (spec §4–§10: Web Dev, Mobile, AI & Intelligent Solutions, Cloud & Enterprise, Business Automation, Testing & QE, DevOps & CI/CD, API & System Integration) — id=`solutions`. Compact card grid, 8 cards — one per H2 category across §4–§10 (Web Development, Mobile Application Development, AI & Intelligent Solutions, Cloud & Enterprise Solutions, Business Automation, Software Testing & Quality Engineering, DevOps & CI/CD, API & System Integration) — each card: icon + title + one-line description (the section's intro sentence). Click/tap "Show more" expands an accordion revealing that category's full sub-bullet list in place.
